...
shinyServer(function(input, output) {
    ...
    output$MosaicPlot <- renderPlot({
        ...
        if(input$caltype=='This'){
            library(dplyr)
            df2=data %>%group_by(Segment) %>%summarise(agg=sum(year2)) %>%mutate(segpct = agg / sum(agg)*100)
            #drop agg
            df2=df2%>%select(-agg)
            #Pivot table of This and segment
            temp_table=data%>%group_by(This,Segment)%>%summarise(agg2=sum(year2))%>%group_by(Segment)%>%mutate(percent=agg2/sum(agg2)*100)%>%select(-agg2)%>%tidyr::spread(Segment,percent)
            #Transpose of it
            temp_table_t = setNames(data.frame(t(temp_table[,-1])),temp_table[,1])
            colnames(temp_table_t)=temp_table$This
            #merge with df2
        ...
        }else if (input$caltype=='That'){
            library(dplyr)
            df3=data %>%group_by(Segment) %>%summarise(agg=sum(year2)) %>%mutate(segpct = agg / sum(agg)*100)
            #drop agg
            df3=df3%>%select(-agg)
            #Pivot table of This and segment
            temp_table2=data%>%group_by(That,Segment)%>%summarise(agg2=sum(year2))%>%group_by(Segment)%>%mutate(percent=agg2/sum(agg2)*100)%>%select(-agg2)%>%tidyr::spread(Segment,percent)
            #Transpose of it
            temp_table_t2 = setNames(data.frame(t(temp_table2[,-1])),temp_table2[,1])
            colnames(temp_table_t2)=temp_table2$That
            #merge with df2
        ...
        df3=data %>%group_by(Segment) %>%summarise(a=sum(year2),b=sum(year1))%>%mutate(growth =((a/b)-1) *100)
        ...
        library(ggplot2)
        p2=ggplot(dfm1, aes(ymin = ymin, ymax = ymax,xmin = xmin, xmax = xmax, fill = variable,order=-ymin))+ geom_rect(colour = I("white"))+ theme_minimal()+ theme(panel.grid.major = element_blank(),panel.grid.minor =element_blank(),legend.position = "bottom",legend.key.size=unit(0.3, "cm"))+ scale_fill_manual(values=c("#CD5C5C", "#F08080", "#FA8072","#E9967A","#FFA07A"))+ labs(x = NULL, y = NULL,fill = NULL)+geom_text(data=subset(dfm1,value!=0),aes(x = xtext, y = ytext,label = ifelse(Segment == "A", paste(variable," - ", value, "%", sep = ""), paste(value,"%", sep = ""))), size = 2.5,color="black")
